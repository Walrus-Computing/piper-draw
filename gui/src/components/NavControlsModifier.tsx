...
export function NavControlsModifier({
  ...
  useEffect(() => {
    ...
    // Alt+Drag = orbit. MIDDLE and RIGHT are "the other gesture": pan when
    // LEFT orbits, orbit when LEFT pans — same as Shift+LEFT. (THREE.MOUSE.ROTATE
    // is three.js' name for the orbit gesture.)
    const apply = (alt: boolean) => {
      ...
      if (navStyle === "rotate") {
        // Drag = orbit, Shift+Drag = pan (handled by OrbitControls' built-in swap).
        controls.mouseButtons.LEFT = THREE.MOUSE.ROTATE;
        controls.mouseButtons.MIDDLE = THREE.MOUSE.PAN;
        controls.mouseButtons.RIGHT = THREE.MOUSE.PAN;
      } else {
        // Drag = pan, Shift+Drag = orbit (built-in swap), Alt+Drag = orbit (manual).
